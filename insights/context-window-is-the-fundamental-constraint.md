...
Claude's 200k token context window is actually ~120k usable before performance degrades. This single constraint explains every effective AI coding practice: subagents isolate research into separate context, `/clear` resets between unrelated tasks, specs files persist knowledge across context resets, and one-objective-per-conversation keeps focus sharp.

...
The practical implication: common failure patterns (kitchen-sink sessions, correction loops, over-specified CLAUDE.md, trust-then-verify gaps, infinite exploration) are all context management failures. Fixing any of them means respecting the fundamental constraint. This is why [[declarative-beats-imperative-for-agents]] matters operationally — verbose step-by-step instructions waste tokens that could carry actual working context. The most direct architectural response is [[one-session-per-contract-beats-long-running-agents]] — using task contracts as session boundaries so cross-contract context bloat never accumulates in the first place.

...
This is [[agent-memory-preserves-institutional-knowledge]] made concrete for a specific domain — the error log isn't just an audit trail, it's a learning mechanism. Without it, [[speed-without-feedback-amplifies-errors]] — agents perpetuate identical mistakes indefinitely because they lack the self-correction feedback loop humans have naturally. Applied to enterprise data, [[context-layers-must-be-living-systems]] extends this pattern: when a data agent gives wrong answers, those corrections should feed back into the context layer itself, not just the agent's memory. The approach parallels [[evolving-summaries-beat-append-only-memory]]: Dash maintains curated knowledge rather than appending raw error logs. The six layers of context Dash uses (schema definitions, human annotations, proven query patterns, institutional docs via MCP, error-correction learnings, runtime database introspection) is a domain-specific implementation of [[three-layer-ai-stack-memory-search-reasoning]], showing how the abstract Memory-Search-Reasoning stack manifests in a real product.

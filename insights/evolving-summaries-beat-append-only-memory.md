...
The write path makes this concrete: save raw conversation, extract atomic facts, classify by category, pull existing summary, evolve summary with new facts, save. This is the mechanism behind what [[agent-memory-preserves-institutional-knowledge]] describes at scale — the "daily changelogs and decision logs" only stay useful if they're periodically consolidated into evolving summaries. It's also why [[specs-are-external-memory-surviving-context-resets]] works: spec files are living documents that get rewritten as understanding deepens, not append-only logs that grow stale.

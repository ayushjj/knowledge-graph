...
Agents already know `cat`, `grep`, `mv`, `mkdir`. Users can inspect and edit what agents create. File paths are self-documenting — `/projects/acme/notes/` beats `SELECT * FROM notes WHERE project_id = 123`. iCloud and Dropbox handle multi-device sync for free.

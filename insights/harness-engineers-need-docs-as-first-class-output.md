...
Documentation becomes the system of record, structured as a map rather than an encyclopedia. A short `AGENTS.md` (~100 lines) serves as a table of contents pointing to deeper sources in a structured `docs/` directory — the same principle behind [[claude-md-should-be-routing-table-not-knowledge-base]], where CLAUDE.md works best as a minimal IF-ELSE directory rather than a monolithic knowledge dump. Knowledge in Google Docs, Slack, or people's heads "effectively doesn't exist" to Codex. This is [[specs-are-external-memory-surviving-context-resets]] taken to its logical extreme — not just specs surviving context resets, but the entire knowledge base being agent-navigable by design. It's also why [[files-are-the-universal-agent-interface]]: the harness runs on files the agent can read.
...
The team estimates they built in 1/10th the time of manual coding. The implication: "give Codex a map, not a 1,000-page instruction manual." Karpathy's Autoresearch crystallizes this further: [[harness-quality-beats-model-intelligence]] — the harness (metrics, rollback, scoping, observability) determines performance more than the model. The strongest harness with a mediocre agent beats the strongest agent with a messy harness. This extends to how the harness describes itself: [[metadata-for-llm-consumers-needs-trigger-specs-not-summaries]] — when metadata is consumed by an LLM deciding what to invoke, it should specify activation conditions rather than summarize functionality.

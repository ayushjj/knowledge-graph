...
PageIndex proves this by building a tree index (like a smart table of contents) and having the LLM reason through it like a domain expert would — "this question about interest rates is probably under Monetary Policy" — achieving 98.7% accuracy on FinanceBench versus typical RAG's 70-80%. This is the same principle driving [[agentic-search-beats-rag-for-live-codebases]]: the agent reasons about *where* to look rather than relying on similarity to surface the right chunks. It also connects to why [[context-is-the-product-not-the-model]] — the quality of retrieval depends on how you structure context, not which model you use for embeddings.

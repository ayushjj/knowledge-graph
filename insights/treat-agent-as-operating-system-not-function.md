...
The mental model shift from "agent as stateless function" to "agent as operating system" reframes every architectural decision. Conversation context is RAM — fast and volatile, lost on reset. Persistent memory is the hard drive — indexed, structured, survives sessions. Memory decay is garbage collection — without it, the system drowns in stale data. Tool integrations are I/O management — how the agent reads from and writes to the external world.

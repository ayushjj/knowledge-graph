...
The revealing finding: early versions only stored deployment-level strategies to prioritize security. But user-level strategies had a significant impact on performance, leading to the two-tier structure. This extends [[cross-user-knowledge-transfer-needs-no-fine-tuning]] with a crucial nuance — cross-user transfer works for workflow patterns, but individual preferences need a private layer. It also enriches [[agent-memory-preserves-institutional-knowledge]] by distinguishing which memory is organizational (and should survive personnel changes) versus personal (and should be private and portable).
